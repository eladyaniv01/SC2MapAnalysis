...
class MapAnalyzerPather:
    ...
    def _add_non_pathables_ground(self, grid: ndarray, include_destructables: bool = True) -> ndarray:
        nonpathables = self.map_data.bot.structures.exclude_type({UnitID.SUPPLYDEPOTLOWERED})
        nonpathables.extend(self.map_data.bot.enemy_structures.exclude_type({UnitID.SUPPLYDEPOTLOWERED}))
